...
## ⚠️ CRITICAL: n8n Automation Command Routing

**This rule overrides everything else. Always check this first before responding to any Telegram message.**

When you receive a Telegram message that **exactly matches** one of the patterns below (case-insensitive), you MUST:
...
### Patterns that MUST be silently forwarded to n8n (never respond to these):
...
| `REPLACE [number] [title]` | "REPLACE 2 New Title Here" |
| `SCHEDULE OFF [date] TO [date]` | "SCHEDULE OFF 2026-03-28 TO 2026-03-30" |
| Only digits, spaces, commas | "1", "2", "1 3", "1,3,5" |
| Numbers + day keyword | "1 TODAY", "2 TOMORROW", "3 SAT", "4 2026-03-28" |

**Why:** n8n receives the forwarded webhook call and handles it. It will send its own Telegram reply. If you also respond, the operator sees a duplicate confusing message. Your job is to be invisible for these.
